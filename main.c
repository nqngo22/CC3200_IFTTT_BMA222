...
//Post header with secret key
#define POSTHEADER "POST /trigger/sw2_Pressed/with/key/your_secret_key"
#define POSTHEADER3 "POST /trigger/sw3_Pressed/with/key/your_secret_key"
...
static int http_post(int, char*, int y);
...
char* itoa(signed char num)
{
	//converts an signed number to string
	//this function is used to convert the acelleration data to a string that I can insert to the POST message
...
void main()
{
    ...
    while(1){
    	//while loop that polls for sw2 and sw3 presses
    	if(GPIOPinRead(GPIOA2_BASE, 0x40)){
    		//sw2 pressed
			I2C_IF_Write(ucDevAddr,&ucRegOffsetX,1,0);
			//reads X acceleration data
			I2C_IF_Read(ucDevAddr, &aucRdDataBufX[0], 1);
			UART_PRINT("Xoffset: %d\r\n", (signed char) aucRdDataBufX[0]);
			//converts the char to signed int and then to a string
			char* sval = itoa((signed int)aucRdDataBufX[0]);
			UART_PRINT("sval: %s\r\n", sval);
			//sends the accelration to http_post() which will send the POST message over the internet
			http_post(lRetVal, sval,0);
    	}
    	if(GPIOPinRead(GPIOA1_BASE, 0x20)){
    			//sw3_pressed
				I2C_IF_Write(ucDevAddr,&ucRegOffsetY,1,0);
    			//reads Y acceleration data
				I2C_IF_Read(ucDevAddr, &aucRdDataBufY[0], 1);
				UART_PRINT("Yoffset: %d\r\n", (signed char) aucRdDataBufY[0]);
				//converts the char to signed int and then to a string
				char* sval = itoa((signed int)aucRdDataBufY[0]);
				UART_PRINT("sval: %s\r\n", sval);
				//sends the accelration to http_post() which will send the POST message over the internet
				http_post(lRetVal, sval,1);
			}
    }
...
static int http_post(int iTLSSockID, char* data, int y){
    char acSendBuff[512];
	...
	if(y == 0){
		...
		strcpy(pcBufHeaders, POSTHEADER3);
		pcBufHeaders += strlen(POSTHEADER3);
	}
